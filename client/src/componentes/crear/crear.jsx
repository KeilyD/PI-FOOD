...
export default function Crear() {
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      input.resumen.length === 0 ||
      input.puntuacion.length === 0 ||
      input.nivel_de_comida_saludable.length === 0 ||
      input.guardar.length === 0||
      input.paso_a_paso.length === 0
    ) {
  ...
  function validate(input) {
    ...
    if (input.nivel_de_comida_saludable < 0) {
      errors.nivel_de_comida_saludable =
        "nivel_de_comida_saludable No puede ser negativo";
    }

    if (
      input.imagen !== "" &&
      !/^(ftp|http|https):\/\/[^ "]+$/.test(input.imagen)
    ) {
    ...
    if (input.paso_a_paso.length < 10) {
      errors.paso_a_paso =
        "las instrucciones deben tener por lo menos 10 caracteres";
    }
  ...
  const handleSelect = (e) => {
    setinput({
      ...input,
      dieta: input.dieta.includes(e.target.value)
        ? input.dieta
        : [...input.dieta, e.target.value],
    });
  };

  const handle_button_hola = (e) => {
    e.preventDefault();
    history.push("/");
  };
  ...
  return (
    <div className={style.mainContainer}>
      <div>
        <button className={style.botones} onClick={handle_button_home}>
          Volver
        </button>
        <button className={style.botones} onClick={handle_button_hola}>
          Inicio
        </button>
      </div>
      <div className={style.mainForm}>
        <form className={style.form}>
          ...
          <Formulario
            ...
            type="number"
            value={input.nivel_de_comida_saludable}
            handle_function={handle_input_change}
            error_control={errors}
          />
