...
export default function Home() {
  ...
  for (
    let i = 1;
    i <= Math.ceil(todasLasRecetas.length / recetasPorPagina);
    i++
  ) {
  ...
  return (
    <div className={styles.home_box}>
      <div className={styles.btns_box}>
        ...
        <button onClick={crear_recetas_boton} className="elementNB">
          Crear Receta
        </button>
        {
          <select onChange={handle_ordenar_por_nombre}>
            <option value="default" className="elementNB" hidden>
        ...
        }
        {
          <select onChange={handle_ordenar_por_puntuacion}>
            <option value="default" className="elementNB" hidden>
              Ordenar recetas por Puntuacion
            </option>
